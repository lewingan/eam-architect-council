...
### 3. Key Entities
| Entity | Source System | Description | Key Fields |
|--------|--------------|-------------|------------|
| Work Order | SAP PM (AUFK/AFIH) | Maintenance task to be executed | order_id, order_type, priority, status, planned_start, planned_end |
| Functional Location | SAP PM (IFLO) | Physical location in plant hierarchy | func_loc_id, description, plant, area |
| Equipment | SAP PM (EQUI) | Individual asset/machine | equipment_id, description, func_loc_id, criticality |
| Work Center | SAP PM (CRHD) | Group of technicians/resources | work_center_id, capacity, plant |
| Operation | SAP PM (AFVC) | Individual task within a work order | operation_id, work_order_id, duration, work_center_id |
| Maintenance Plan | SAP PM (MPOS) | Recurring schedule definition | plan_id, cycle, strategy, call_horizon |
...
### 6. Interface Design
#### 6.1 Inputs
- SAP PM: Work orders, operations, work centers (via RFC/BAPI or OData)
...
### 10. Open Items
- [ ] Confirm OData vs BAPI for SAP integration
